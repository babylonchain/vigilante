package types_test

import (
	"bytes"
	"encoding/hex"
	"testing"

	"github.com/babylonchain/babylon/btctxformatter"
	"github.com/babylonchain/vigilante/types"
	"github.com/btcsuite/btcd/btcutil"
	"github.com/btcsuite/btcd/wire"
	"github.com/stretchr/testify/require"
)

// Two real txs that carry a complete Babylon checkpoint
var (
	tx1Hex = ""
	tx2Hex = ""
)

func toWrappedTx(txHex string) (*btcutil.Tx, error) {
	decodedTx, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	tx := wire.NewMsgTx(0)
	err = tx.Deserialize(bytes.NewReader(decodedTx))
	if err != nil {
		return nil, err
	}
	wrappedTx := btcutil.NewTx(tx)
	return wrappedTx, nil
}

func TestNewCkptSegment(t *testing.T) {
	// The transaction hex corresponds to the tat "bbt0" (in string format)
	// which translates to the below bytes
	tag := []byte{98, 98, 116, 48}
	version := btctxformatter.CurrentVersion

	wTx1, err := toWrappedTx(tx1Hex)
	require.NoError(t, err)
	wTx2, err := toWrappedTx(tx2Hex)
	require.NoError(t, err)

	seg1 := types.NewCkptSegment(tag, version, nil, wTx1)
	require.NotNil(t, seg1)
	seg2 := types.NewCkptSegment(tag, version, nil, wTx2)
	require.NotNil(t, seg2)

	ckpt, err := btctxformatter.ConnectParts(version, seg1.Data, seg2.Data)
	require.NoError(t, err)
	require.NotEmpty(t, ckpt)
}
