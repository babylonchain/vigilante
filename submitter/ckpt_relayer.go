...
func (s *Submitter) ConvertCkptToTwoTxAndSubmit(ckpt *ckpttypes.RawCheckpointWithMeta) error {
	...
	if amount.ToUnit(btcutil.AmountSatoshi) < txfee*2 {
		return errors.New("insufficient fees")
	}
...
func (s *Submitter) pickHighUTXO() (*btcjson.ListUnspentResult, error) {
	...
	// sort utxos by amount in the descending order
...
func (s *Submitter) buildTxWithData(
	...
	log.Debugf("balance of input: %v satoshi, tx fee: %v satoshi, output value: %v",
		int64(amount.ToUnit(btcutil.AmountSatoshi)), int64(s.btcWallet.Cfg.TxFee.ToUnit(btcutil.AmountSatoshi)), int64(change))
